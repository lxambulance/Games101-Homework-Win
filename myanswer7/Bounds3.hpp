...
inline bool Bounds3::IntersectP(const Ray& ray, const Vector3f& invDir,
    ...
    float tEnter = std::max({(dirIsNeg[0] ? tMin.x : tMax.x),
                             (dirIsNeg[1] ? tMin.y : tMax.y),
                             (dirIsNeg[2] ? tMin.z : tMax.z)}),
          tExit = std::min({(dirIsNeg[0] ? tMax.x : tMin.x),
                            (dirIsNeg[1] ? tMax.y : tMin.y),
                            (dirIsNeg[2] ? tMax.z : tMin.z)});
    return tEnter <= tExit + 1e-8 && tExit >= 0;
}
