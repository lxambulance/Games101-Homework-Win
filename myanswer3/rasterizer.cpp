...
void rst::rasterizer::rasterize_triangle(
  ...
  for (auto& vert : t.v) {
    int tmpx = int(vert.x()), tmpy = int(vert.y());
    x_min = std::min(tmpx, x_min);
    x_max = std::max(tmpx, x_max);
    y_min = std::min(tmpy, y_min);
    y_max = std::max(tmpy, y_max);
  }
    ...
    for (int i = x_min; i <= x_max; ++i)
      for (int j = y_min; j <= y_max; ++j) {
        if (insideTriangle(i, j, t.v)) {
          ...
          // color directly to the frame buffer, pass the color to the shaders
          // first to get the final color; Use: auto pixel_color =
          // fragment_shader(payload);
          auto [alpha, beta, gamma] =
              computeBarycentric2D(i + 0.5f, j + 0.5f, t.v);

          alpha /= t.v[0].w(), beta /= t.v[1].w(), gamma /= t.v[2].w();
          float w = alpha + beta + gamma;

          float z_interpolated =
              (alpha * v[0].z() + beta * v[1].z() + gamma * v[2].z()) / w;
          auto interpolated_color = interpolate(alpha, beta, gamma, t.color[0],
                                                t.color[1], t.color[2], w);
          auto interpolated_normal = interpolate(
              alpha, beta, gamma, t.normal[0], t.normal[1], t.normal[2], w);
          auto interpolated_texcoords =
              interpolate(alpha, beta, gamma, t.tex_coords[0], t.tex_coords[1],
                          t.tex_coords[2], w);
          auto interpolated_shadingcoords = interpolate(
              alpha, beta, gamma, view_pos[0], view_pos[1], view_pos[2], w);
